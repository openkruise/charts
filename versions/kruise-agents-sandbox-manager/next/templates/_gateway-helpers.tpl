{{/*
Envoy configuration content for sandbox-gateway.
Extracted into a named template so the ConfigMap checksum only depends on the
actual envoy config, not on chart/version labels. This avoids unnecessary
sandbox-gateway restarts on chart upgrades when the config itself is unchanged.
*/}}
{{- define "sandbox-gateway.envoyConfig" -}}
admin:
        ...
        - filters:
            - name: envoy.filters.network.http_connection_manager
              typed_config:
                ...
                stream_idle_timeout: {{ .Values.gateway.envoy.streamIdleTimeout }}
                access_log:
                ...
                http_filters:
                  - name: envoy.filters.http.golang
                    typed_config:
                      ...
                      library_id: {{ .Values.gateway.envoy.golangFilter.libraryId }}
                      library_path: {{ .Values.gateway.envoy.golangFilter.libraryPath }}
                      plugin_name: {{ .Values.gateway.envoy.golangFilter.pluginName }}
                      plugin_config:
                        "@type": type.googleapis.com/xds.type.v3.TypedStruct
                        value:
                          host-header-name: {{ .Values.gateway.envoy.pluginConfig.hostHeaderName }}
